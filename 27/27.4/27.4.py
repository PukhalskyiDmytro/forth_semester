...
def get_session(environ):
    ...
    if sid not in SESSIONS:
        sid = str(uuid.uuid4())
        SESSIONS[sid] = {"user": None, "numbers": [], "previous": None, "changes": 0, "finished": False}
...
<title>{title}</title>
<style>
body {{ font-family: Arial, sans-serif; width: 800px; margin: 30px auto; background: #f4f4f4; }}
...
def send(start_response, sid, body, status="200 OK"):
    start_response(status, [("Content-Type", "text/html; charset=utf-8"), ("Set-Cookie", f"sid={sid}; Path=/")])
    return [body.encode("utf-8")]


def redirect(start_response, sid, location):
    start_response("302 Found", [("Location", location), ("Set-Cookie", f"sid={sid}; Path=/")])
    return [b""]
...
<h3>Задача 27.4</h3>
<p>Задана непорожня послідовність ненульових цілих чисел, за якою йде 0.</p>
<p>Потрібно визначити кількість змін знаку в цій послідовності.</p>
...
def task_page(session, error=""):
    numbers_text = ", ".join(str(x) for x in session["numbers"]) or "ще не введено"
    error_html = f"<p class='error'>{escape(error)}</p>" if error else ""
    if session["finished"]:
        result_html = f"<p class='ok'>Введення завершено.</p><p><b>Кількість змін знаку:</b> {session['changes']}</p>"
    else:
        result_html = f"<p><b>Поточна кількість змін знаку:</b> {session['changes']}</p>"
...
def process_number(session, params):
    if params.get("action", ["add"])[0] == "reset":
        session["numbers"] = []
